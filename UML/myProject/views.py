...
from .forms import PerformanceQueryForm
...
def professor_valid(request, professor_id):
    if request.method == "POST":
        ...
        if action == "f4":
            ...
            return render(
                request,
                "course_sections_and_students.html",
                {"course_sections_and_students": course_sections_and_students},
            )
        ...
        return render(
            request, "professor_form_valid.html", {"professor_id": professor_id}
        )
    ...
    if request.method == "POST":
        ...
        if student_exists:
            ...
            if user is not None:
                ...
                # Incorrect password
                return render(
                    request, "student_form.html", {"invalid_student_id": False}
                )
        else:
...
def instructor_performance(request):
    if request.method == "POST":
        form = PerformanceQueryForm(request.POST)
        if form.is_valid():
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT i.name, COUNT(DISTINCT t.sec_id) AS number_of_sections,
                           COUNT(DISTINCT k.student_id) AS number_of_students,
                           COALESCE(SUM(f.amount), 0) AS total_funding,
                           COUNT(DISTINCT p.publication_id) AS number_of_publications
                    FROM instructor i
                    LEFT JOIN teaches t ON i.id = t.teacher_id
                    LEFT JOIN takes k ON t.course_id = k.course_id AND k.year = %s
                    LEFT JOIN instructor_funding f ON i.id = f.instructor_id AND f.funding_year = %s
                    LEFT JOIN instructor_publications p ON i.id = p.instructor_id
                    WHERE i.name = %s AND t.semester = %s
                    GROUP BY i.name
                """,
                    [year, year, name, semester],
                )
                result = cursor.fetchall()

            return render(
                request, "instructor_performance.html", {"performance_data": result}
            )
    else:
        form = PerformanceQueryForm()

    return render(request, "instructor_performance_form.html", {"form": form})
